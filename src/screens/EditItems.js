...
const Editdata = () => {
  ...
  const [image, setImage] = useState({
    assets: [{uri: route.params.data.imgUrl}],
  });
  const [name, setName] = useState(route.params.data.name);
  const [price, setPrice] = useState(route.params.data.price);
  const [disprice, setDisprice] = useState(route.params.data.disprice);
  const [discription, setDiscription] = useState(route.params.data.discription);
  ...
  const updation = (url) => {
    db.collection('items')
      ...
      .update({
        ...
        imgUrl: route.params.data.imgUrl + '',
      })
      .then(() => {
